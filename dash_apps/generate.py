...
def validate_topic_rules(text, lang='en'):
    ...
    # Argo / müstehcen / küfür içeren konular (hizli on-eleme; AI'a gitmeden).
    import re as _re
    ...
    for root in hard_roots:
        if root.translate(leet) in collapsed:
            return False, t('gen_val_inappropriate', lang)

    word_bound = [
        'sik', 'piç', 'pic', 'göt', 'seks', 'sex', 'dick', 'shit',
    ]
    for p in word_bound:
...
def get_base_prompt(user_request_text, word_count=1500, real_sources=None,
    ...
    - İstek konusunu ciddi, akademik bir konu olarak ele al.
